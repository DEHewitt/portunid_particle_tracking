...
} else if (species == "gmc" & direction == "backwards" | species == "bsc" & direction == "backwards"){
  # what to do about backwards dates?
  # i don't think anything needs to be done - later on when creating the settled df this is handled
}
...
if (direction == "forwards"){
  ...
  saveRDS(particles.final, file = paste(file.path, species, direction, "processed", paste(species, min(year(particles$rel_date)), unique(particles$ocean_zone), direction, "final_points.rds", sep = "_"), sep = "/"))
...
# create a df of settled points (dd = dd.cutoff, alive and on shelf (bathy < 200))
if (direction == "forwards"){
  ...
  if (species == "gmc" | species == "bsc"){
    particles.settled <- particles.final %>% 
      ...
      filter(obs == min(obs)) %>% 
      ungroup() %>%
      mutate(estuary = case_when(rel_lat == -18.541 ~ "Hinchinbrook Island",
...
if (species == "gmc" | species == "bsc"){
  saveRDS(particles.settled, file = paste(file.path, species, direction, "processed", paste(species, min(year(particles$rel_date)), unique(particles$ocean_zone), direction, "settled.rds", sep = "_"), sep = "/"))
} else {
  # spanner crab files
}
