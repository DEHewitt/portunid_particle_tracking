settlement_corr <- function(data, type, group){
  # data is the joined settlement and cpue
  # type is either landings or cpue
  # group either estuary or egf
  ...
  if (group == "estuary"){
    if (type == "cpue"){
      ...
      corr <- data %>%
        split(.$estuary) %>%  
        map(~ corr.test(.$landings, .$settlement,
                        use = "pairwise",
                        method = "pearson",
                        adjust = "bonferroni",
                        alpha = 0.05,
      ...
      corr.1 <- data %>%
        split(.$estuary) %>%  
        map(~ corr.test(.$landings, .$settlement.1,
                        use = "pairwise",
                        method = "pearson",
                        adjust = "bonferroni",
                        alpha = 0.05,
      ...
      corr.2 <- data %>%
        split(.$estuary) %>%  
        map(~ corr.test(.$landings, .$settlement.2,
                        use = "pairwise",
                        method = "pearson",
                        adjust = "bonferroni",
                        alpha = 0.05,
      ...
      corr.3 <- data %>%
        split(.$estuary) %>%  
        map(~ corr.test(.$landings, .$settlement.3,
                        use = "pairwise",
                        method = "pearson",
                        adjust = "bonferroni",
                        alpha = 0.05,
