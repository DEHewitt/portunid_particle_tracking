...
u_files <- list.files("/srv/scratch/z3374139/BRAN_AUS/", pattern="Ocean_u_", full.names = T)
...
v_files <- list.files("/srv/scratch/z3374139/BRAN_AUS/", pattern="Ocean_v_", full.names = T)
...
temp_files <- list.files("/srv/scratch/z3374139/BRAN_AUS/", pattern="Ocean_temp_", full.names = T)
...
for (i in (1:(length(u_files_1)-1))) {
  ...
  dat1b <- tidync(u_files_2[i+1]) %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
    hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>% 
    group_by(Month,xu_ocean ,yu_ocean ) %>% summarise(across(where(is.double),~ mean(.x, na.rm = TRUE))) %>%
    filter(xu_ocean >150 & yu_ocean>-31 & xu_ocean <160 & yu_ocean < -20)
...
dat2b <- tidync(v_files_2[i+1]) %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
  hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>% 
  group_by(Month,xu_ocean ,yu_ocean ) %>% summarise(across(where(is.double),~ mean(.x, na.rm = TRUE))) %>%
  filter(xu_ocean >150 & yu_ocean>-31 & xu_ocean <160 & yu_ocean < -20)
...
dat3b <- tidync(temp_files_2[i+1]) %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
  hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>% 
  group_by(Month,xt_ocean ,yt_ocean ) %>% summarise(across(where(is.double),~ mean(.x, na.rm = TRUE))) %>%
  filter(xt_ocean >150 & yt_ocean>-31 & xt_ocean <160 & yt_ocean < -20)
...
ggplot(full_dat, aes(x=xu_ocean, y=yu_ocean)) +
  ...
  ggtitle(paste0("Dec/Jan Spawning Season ",floor(full_dat$Year[1])))
