...
files <- list.files("/srv/scratch/z3374139/portunid_particle_tracking/spanner/forwards", pattern = ".nc", full.names = TRUE)

mydata2 <- readRDS("/srv/scratch/z3374139/portunid_particle_tracking/spanner/forwards/processed/spanner_forwards_master_final.rds")
...
mydata <- tidync(files[i]) %>% hyper_tibble()
particles.info <- nc_open(files[i])
direction = "forwards"
species <- "spanner"
...
saveRDS(mydata, paste0("/srv/scratch/z3374139/portunid_particle_tracking/spanner/forwards/full_paths/Output_",as.character(i),".rds"))
