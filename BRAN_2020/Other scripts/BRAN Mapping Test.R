...
dat1 <- tidync("AUS/Ocean_u_1994_01.nc") %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
...
dat2 <- tidync("AUS/Ocean_v_1994_01.nc") %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
  hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>% 
  group_by(Month,xu_ocean ,yu_ocean ) %>% summarise(across(where(is.double),~ mean(.x, na.rm = TRUE))) %>%
  filter(xu_ocean >143 & yu_ocean>-35)
head(dat2)

dat3 <- tidync("AUS/Ocean_temp_1994_01.nc") %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
  hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>% 
