...
library(lubridate)
library(metR)
temp <- tidync("C:/Users/htsch/Documents/GitHub/portunid_particle_tracking/BRAN_2020/AUS/Ocean_temp_1993_01.nc") %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
...
u_dat <- tidync("C:/Users/htsch/Documents/GitHub/portunid_particle_tracking/BRAN_2020/AUS/Ocean_u_1993_01.nc") %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
  hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>%
  filter(Day == 1 & Month ==1) %>%
  filter(xu_ocean >150 & yu_ocean>-31 & xu_ocean <157 & yu_ocean < -21)
v_dat <- tidync("C:/Users/htsch/Documents/GitHub/portunid_particle_tracking/BRAN_2020/AUS/Ocean_v_1993_01.nc") %>% hyper_filter(st_ocean  = st_ocean  == 2.5) %>%
  hyper_tibble() %>% mutate(Time = as.Date(Time, origin="1979-01-01"), Day = day(Time), Month = month(Time), Year=year(Time)) %>%
  filter(Day == 1 & Month ==1) %>%
  filter(xu_ocean >150 & yu_ocean>-31 & xu_ocean <157 & yu_ocean < -21)
...
p2 <- ggplot(full_dat)+ geom_tile(data=temp, aes(x=xt_ocean, y=yt_ocean, fill=temp))+
  ...
  coord_sf(xlim = c(150, 156), ylim = c(-30, -22), expand = FALSE)+
  scale_fill_viridis_c(option="turbo", name = "Temperature (°C)")+
...
### INSET MAP CODE
F1 <- p1 + inset_element(p3, 
  left = 0, right=0.45,
  bottom = 0, top=0.4
)
F1 + p2 +  plot_annotation(tag_levels = list(c('a','','b')))
ggsave("Figure 1.png", dpi = 600, units = "cm", width=21, height=14.8)
