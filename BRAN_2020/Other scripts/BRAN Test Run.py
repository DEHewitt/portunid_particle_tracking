...
@author: hayde
"""
from parcels import FieldSet, Field,  ParticleSet, JITParticle, Variable, DiffusionUniformKh, random, plotTrajectoriesFile #AdvectionRK4,
...
#                   'temp': sorted(glob('/srv/scratch/z3374139/BRAN_AUS/AUS/Ocean_temp_*')),
...
possible_locations = pd.read_csv("/Users/htsch/Documents/GitHub/portunid_particle_tracking/BRAN/AUS/spanner_possible_locations.csv")
...
start_time = datetime(year_array[array_ref],1, 1)
end_time = datetime(year_array[array_ref],2, 28)  #year, month, day,
...
#out_file = str("/srv/scratch/z5278054/portunid_particle_tracking/spanner/forwards/spanner_") +str(year_array[array_ref])+'_'+str("forward")+str(array_ref)+'.nc'
...
kernels = SampleInitial + pset.Kernel(AdvectionRK4_3D_alternative) + SampleAge+ SampleTemp + SampleDepth+ SampleDistance + DiffusionUniformKh + SampleBathy + larvalBuoyancy
...
#### calendar check
import xarray as xr
ds_disk = xr.open_dataset("/Users/htsch/Documents/GitHub/portunid_particle_tracking/BRAN/AUS/Ocean_v_1994_01.nc")
ds_disk
