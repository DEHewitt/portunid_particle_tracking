...
mydata <- readRDS("Output/spanner_forwards_10_master_settled.rds")
...
head(nsw_sum)
write_csv(nsw_sum, "test_out2.csv")
nsw_sum <- read_csv("test_out2.csv")
...
write_csv(full_dat, "Predicted recruitment full season mortalityx10.csv")
...
ggsave("C:/Users/htsch/Desktop/Snapper Crab/Predicted v CPUE_Mort10.png", dpi=600, units="cm", height = 14.8, width=21)
ggsave("C:/Users/htsch/Desktop/Snapper Crab/Predicted v CPUE_Mort10.pdf", dpi=600, units="cm", height = 14.8, width=21)
...
full_dat_standardised_longX_Nov <- full_dat_standardisedX_Nov %>% select(Year, Month, standardised_lagged_Predicted_recruitment, CPUE_relative1, CPUE_relative2) %>%
  pivot_longer(3:5) %>% mutate(Month=11)
...
ggplot(full_dat_standardised_longX_Nov, aes(Year, value, col=name)) + geom_point() + geom_line()+
  ...
  theme(axis.text = element_text(colour="black", size=12),
        ...
        legend.title = element_text(face="bold",size=14),
        legend.text = element_text(size=12)) +
  scale_color_manual(values=c("red","blue","black"), name="Data\nSource", labels = c("Relative CPUE1","CPUE2", "Predicted Recruitment\nto Fishery"))

#ggsave("Predicted v CPUE November spawning only.png", dpi=600, units="cm", height = 14.8, width=21)
cor.test(full_dat_standardisedX_Nov$standardised_lagged_Predicted_recruitment, full_dat_standardisedX_Nov$CPUE_relative2)
...
ggsave("C:/Users/htsch/Desktop/Snapper Crab/Predicted v CPUE December spawning only_Mort10.png", dpi=600, units="cm", height = 14.8, width=21)
ggsave("C:/Users/htsch/Desktop/Snapper Crab/Predicted v CPUE December spawning only_mort10.pdf", dpi=600, units="cm", height = 14.8, width=21)
...
M10 <- full_dat_standardised_longX_Dec
