# Compare Mooring and Bran - v only
# Coffs 100
...
ggplot(all_dat, aes(x=date, y=UCUR_BRAN)) + geom_line()+
  geom_line(aes(y=UCUR_Mooring), col="red")
...
# 
# 
# all_dat <- all_dat %>% mutate(Vel_BRAN = sqrt((UCUR_BRAN^2 + UCUR_BRAN^2)), 
#                               Vel_Mooring = sqrt((UCUR_Mooring^2 + UCUR_Mooring^2)))
# 
...
all_dat$Vel_BRAN_smooth = zoo::rollmean(all_dat$UCUR_BRAN, 7, fill=NA)
all_dat$Vel_Mooring_smooth = zoo::rollmean(all_dat$UCUR_Mooring, 7, fill=NA)
...
# 
# 
# all_dat <- all_dat %>% mutate(KE_BRAN = sqrt(UCUR_BRAN^2 + UCUR_BRAN^2), 
#                               KE_Mooring = sqrt(UCUR_Mooring^2 + UCUR_Mooring^2))
# 
...
all_dat$KE_BRAN_smooth = zoo::rollmean(all_dat$UCUR_BRAN, 7, fill=NA)
all_dat$KE_Mooring_smooth = zoo::rollmean(all_dat$UCUR_Mooring, 7, fill=NA)
...
taylor.diagram(ref=all_dat_EAC4700$KE_Mooring_smooth, model = all_dat_EAC4700$KE_BRAN_smooth, normalize = F, pos.cor=F, add=F)
...
taylor.diagram(ref=all_dat_Coffs100$Vel_Mooring_smooth, model = all_dat_Coffs100$Vel_BRAN_smooth, normalize = F, pos.cor=F, add=T)
